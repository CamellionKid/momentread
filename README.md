...
opencode 会话按运行时隔离：每个讨论会话记录其所属运行时，切换运行时后自动从已保存的讨论背景创建新会话，不会跨运行时复用；中断或异常退出的会话会被标记为不可接续，下次提问自动换新会话。

**核对实际调用的版本。** 本机可能存在多个 opencode 安装（例如 Homebrew 与应用实际调用的二进制版本不同）。界面「AI 连接与本地数据」对话框显示的版本号来自 MomentRead 实际调用的二进制；终端里直接运行 `opencode --version` 看到的可能是另一个安装，两者不一致时以对话框为准。

**「已登录」不等于「可用」。** 对话框中 opencode 状态分两阶段：先报告安装与登录凭据，完成一次真实回答后才显示「已完成实际调用」。模型列表能加载、按钮能切换都不代表真实调用可用。真实验证可运行：

```sh
npm run test:live:opencode -- first
```

该命令真实调用当前配置的 opencode 模型并消耗其额度，返回完整中文结果才算通过。

**错误提示怎么看。** opencode 运行失败时界面会显示脱敏后的原因与下一步建议，内部按认证失败、额度/限流、模型不存在、配置或 agent 错误、超时、进程异常退出分类；完整凭据、请求正文与内部路径不会进入界面。网关类免费额度（如限流 429、间歇 500）恢复后重试即可；提示模型不存在时以下拉列表中的模型名为准。判断成功以进程正常退出且结果完整为准，不凭中间事件。
...
| opencode 显示已登录但不回复 | 「已登录」不代表可用；用 `npm run test:live:opencode -- first` 做真实调用验证，并按界面脱敏错误分类处理      |
| 切换运行时后继续会话失败    | 会话按运行时隔离，切换后会自动新建会话；旧会话记录保留但不会被另一运行时用                                  |
