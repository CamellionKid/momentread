...
更新日期：2026-09-10。来源：已确认的 [PRD](PRD.md)、[视觉基线](design/README.md)、[ThoughtDAG 复用研究](research/thoughtdag-reuse.md)，以及用户对“独立网页、优先 Claude Code 桥接”的认可。本文件描述后续开发方向，当前仓库已有独立的交互设计原型，生产后端和真实阅读数据尚未接入。
...
- 已完成：需求、静态视觉、ThoughtDAG 固定提交研究，以及 D1 的交互设计原型。原型采用 React 19.2、JavaScript、Vite 6.4.2，运行时验证为 Node 22.23.0；这些是设计原型版本，不代表生产工程建议已全部冻结。
- 尚未接入：第三方源码迁入、CLI、真实 EPUB、原著检索、数据库、完整小结版本历史与重启恢复。HTML 原型已能导出确认的小结；正式每日统计与 AI 总结仍待开发。
- 下一步：根据用户先设计 UI／UX 的安排，D1 先于 D0 后端验证完成。后续回到 D0，准备生产脚手架与最小 Claude Code 验证，保留已验证的交互语义；不重复推翻已确认布局。
- 本轮设计证据：[UI／UX 说明](design/UI-UX.md)、[原型启动](../prototype/README.md)、[浏览器与视觉验收](../prototype/design-qa.md)。原型中没有新增生产 API、数据库 schema 或网络模型调用。
