...
export function createApp(services:Services){
 ...
 const workspace=(id:string)=>{book(id);let w=store.get('workspaces',id);if(!w){w={id,bookId:id,position:null,activeDiscussionId:null,collapsed:[],fontSize:24,updatedAt:now()};store.put('workspaces',w)}return w};
