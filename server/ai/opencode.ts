...
export function opencodeArguments(request: StartRun, model?: string): string[] {
  const args = ['run', '--format', 'json', '--pure', '--agent', 'momentread'];
...
function exec(binary: string, args: string[], cwd: string): Promise<string> {
  return new Promise((resolve, reject) => execFile(binary, args, {cwd, env: environment(), timeout: 15000, maxBuffer: 4 * 1024 * 1024}, (error, stdout) => error ? reject(error) : resolve(stdout)));
}
function classifyExit(stderrHint: string): {code: string; message: string} {
  if (/401|unauthorized|authentication|not logged in/i.test(stderrHint)) return {code: 'CLI_AUTH_REQUIRED', message: 'AI 服务认证失败，请检查 opencode 的登录状态后重试。'};
  if (/429|rate.?limit/i.test(stderrHint)) return {code: 'CLI_RATE_LIMIT', message: 'AI 服务达到调用频率或额度上限，请稍后重试。'};
  return {code: 'CLI_RUN_FAILED', message: 'AI 运行未正常完成，请检查 opencode 登录和服务状态后重试。'};
}

export function createOpencodeAdapter(options: OpencodeAdapterOptions = {}): ClaudeAdapter {
  const cwd = options.cwd || join(options.dataDir || process.env.MOMENTREAD_DATA_DIR || join(homedir(), 'Library/Application Support/MomentRead'), 'runtime/opencode');
  ...
  const finish = (state: Active) => {
    ...
    if (state.cancelled) emit(state, 'cancelled', {text: state.text, sessionReusable: false, forced: state.forced});
    ...
    else if (state.sawFinish && state.sessionId && state.text.trim() && !state.forced) {
      ...
      else emit(state, 'completed', {text: state.text, ...(structured ? {structuredOutput: structured} : {}), toolResults: [], sessionReusable: true});
    } else emit(state, 'failed', {...classifyExit(''), sessionReusable: false});
  ...
  const handle = (state: Active, message: Record<string, any>) => {
    ...
    if (message.type === 'error') {
      const classified = classifyExit(typeof message.error?.message === 'string' ? message.error.message : '');
      fail(state, classified.code, classified.message);
      return;
    }
  ...
  return {
    ...
    async start(request: StartRun) {
      ...
      const args = opencodeArguments(request, model);
      ...
      const proc = spawn(binary, args, {cwd, env: environment(), stdio: ['ignore', 'pipe', 'pipe']});
      let resolveClose!: () => void;
      const state: Active = {request, proc, queue: new AsyncQueue(), seq: 0, sawFinish: false, cancelled: false, forced: false, closed: false, text: '', parts: new Map(), closePromise: new Promise(resolve => {resolveClose = resolve}), resolveClose};
      ...
      proc.on('close', () => {
        ...
        if (stderr) log(`opencode diagnostic output withheld (${Buffer.byteLength(stderr)} bytes).`);
        if (!state.sessionId && !state.cancelled && !state.failure) state.failure = classifyExit(stderr);
        finish(state);
      });
