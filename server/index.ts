...
const store=createStore(dataDir);const library=createBookLibrary(store,dataDir);const adapter=aiProvider==='opencode'?createOpencodeAdapter({dataDir,model:aiModel}):createClaudeAdapter({dataDir,model:aiModel});
const {app,runs}=createApp({store,library,adapter,learning:createLearningService(store),matching:createMatchingService(store,library),port,ai:{provider:aiProvider,model:aiModel}});
